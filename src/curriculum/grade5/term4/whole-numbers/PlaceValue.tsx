...
const PlaceValue = () => {
    ...
    const [targetDigitIndex, setTargetDigitIndex] = useState<number>(0); // 0 = ones, 1 = tens, etc.
    ...
    const generateProblem = () => {
        // Generate number between 10,000 and 999,999
        const newNum = Math.floor(Math.random() * 900000) + 100000;
        setNumber(newNum);

        // Pick a random digit index (0 to 5)
        ...
        while (distractors.size < 4) {
            // Distractor types:
            // 1. Same digit, different place value
            // 2. Different digit, same place value
            // 3. Just the digit itself (face value)
            ...
            // Fallback if we can't find enough unique ones easily (rare)
    ...
    const handleCheck = (val: number) => {
        if (selectedOption !== null) return; // Prevent multiple guesses per problem
    ...
    // Format number with spaces for readability (e.g. 123 456)
    const formatNumber = (num: number) => {
        return num.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    };

    const digitStr = number.toString();
    // Calculate the index in the string (from left) corresponding to the target power (from right)
    // String index = length - 1 - targetIndex
    const highlightedIndex = digitStr.length - 1 - targetDigitIndex;

    return (
        <div className="max-w-2xl mx-auto p-6">
            <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-bold text-slate-800">Place Value Challenge</h2>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 text-center mb-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {options.map((opt, idx) => {
                        let btnClass = "p-6 rounded-xl text-xl font-bold border-2 transition-all hover:scale-105 ";
